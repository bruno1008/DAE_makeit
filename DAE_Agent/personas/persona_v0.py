...
@dataclass
class PersonaConfig:
    """Configuration for a persona with provider and model settings."""
    provider: str
    model: str
    voice_provider: str
    voice_config: dict
...
class PersonaAssistant:
    """Class to create and manage persona assistants with Vapi API."""
    ...
    def __init__(self, persona_config: PersonaConfig, assistant_name: str):
        self.persona_config = persona_config
        self.assistant_name = assistant_name
        with open('persona_prompt_v0.txt', 'r', encoding='utf-8') as file:
            self.persona_system_prompt = file.read().strip()

    def _build_assistant_config(self) -> dict:
        """Build the assistant configuration dictionary with provider-specific parameters."""
        model_params = MODEL_CONFIGS[self.persona_config.provider]["params"]
        voice_params = VOICE_CONFIGS[self.persona_config.voice_provider]["params"]
        transcriber_params = TRANSCRIBER_CONFIGS[self.persona_config.transcriber_provider]["params"]

        return {
            "name": self.assistant_name,
            "model": {
                "provider": self.persona_config.provider,
                "model": self.persona_config.model,
                "messages": [{"role": "system", "content": self.persona_system_prompt}],
                **model_params
            },
            "voice": {
                "provider": self.persona_config.voice_provider,
                "voiceId": self.persona_config.voice_config["voiceId"],
                "model": self.persona_config.voice_config["model"],
                **voice_params
            },
            "transcriber": {
                "provider": self.persona_config.transcriber_provider,
                "model": self.persona_config.transcriber_model,
                **transcriber_params
            },
            "firstMessage": "Sim, preciso do DAE agora! Senhora idosa em parada no ferry da Praia de São Jacinto, Aveiro. Enfermeiro mandou eu pegar!",
            "firstMessageMode": "assistant-waits-for-user",
            "voicemailMessage": "Preciso de ajuda!",
            "endCallMessage": "Ajudem-me!",
    ...
    def create(self) -> None:
        ...
        try:
            ...
            print(f"Created persona assistant {self.assistant_name} with ID {vapi_assistant_id}")
        except requests.exceptions.RequestException as e:
            print(f"Error creating persona assistant {self.assistant_name}: {str(e)}")
            return

def main():
    """Create the specific persona."""
    provider = "openai"
    model = MODEL_CONFIGS[provider]["models"][0]
    voice_provider = "11labs"
    voice_config = VOICE_CONFIGS[voice_provider]["voices"][0]
    transcriber_provider = "cartesia"
    ...
    PersonaAssistant(persona_config, assistant_name).create()
